class TasksController < ApplicationController
  ...
  def update_params
    params.require(:task).permit(:title, :done, :started, :description).merge(tags: tags)
  end

  def tags
    Array(params.permit(tags: [])[:tags]).map do |tag_name|
      current_user.tags.find_by('lower(name) = ?', tag_name.downcase) || current_user.tags.create(name: tag_name)
    end
  end
end
